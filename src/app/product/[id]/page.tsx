...
import { getProductById, getVisibleProductsBySeller } from "@/lib/firebase/products";
...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: product ? `${product.name} | Shipda` : "Shipda" };
}
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const related = (await getVisibleProductsBySeller(product.sellerId).catch(() => []))
    ...
    <div className="mx-auto max-w-content px-5 py-10 md:px-8 md:py-16">
      <div className="grid grid-cols-1 gap-10 md:grid-cols-2 md:gap-16">
        ...
        <div className="flex flex-col">
          ...
          <p className="mt-3 font-mono text-xl font-semibold text-ink">{formatPrice(product.price)}</p>
      ...
      {related.length > 0 && (
        <div className="mt-24">
          <p className="mb-6 font-display text-xl font-bold text-ink">함께 보면 좋은 상품</p>
