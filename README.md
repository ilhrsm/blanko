...
인스타그램 기반 소상공인·1인 창업자를 위한 완전 무료 쇼핑몰 빌더. 사업자 등록 없이 이메일/비밀번호로 가입하면
`shipda.com/상점아이디` 형태의 나만의 고유 주소가 생기고, 가결제로 바로 판매를 시작할 수 있습니다.
Next.js(App Router) + TypeScript + Tailwind CSS + Firebase로 제작되었습니다.
...
2. **Authentication → Sign-in method**에서 Google 로그인과 이메일/비밀번호 로그인을 모두 활성화합니다.
...
## Vercel 배포 오류: "Couldn't find any pages or app directory"

이 오류는 거의 항상 **저장소 구조 문제**입니다. Vercel은 (Root Directory 설정이 비어있다면) 저장소 최상위에 `package.json`과 `src/app` 폴더가 바로 있어야 인식합니다.

**원인 확인 방법**: GitHub 저장소 페이지를 열었을 때 파일 목록 맨 위에 `package.json`, `next.config.mjs`, `src` 폴더가 바로 보여야 정상입니다. 만약 그 안에 `shipda` 같은 폴더가 한 번 더 감싸고 있다면 (`shipda/package.json`, `shipda/src/...`) 이게 원인입니다.

**해결 방법 1 — 저장소 구조를 고친다 (권장)**
```bash
# 압축을 푼 폴더 자체가 아니라, 그 "안의 내용물"이 저장소 루트로 가야 합니다
cd shipda           # 프로젝트 폴더 안으로 이동
git init
git add .
...
```
즉, `git init`을 프로젝트 폴더 **안에서** 실행해야 하며, 폴더 자체를 통째로 다른 폴더에 넣고 커밋하면 안 됩니다.

**해결 방법 2 — 이미 잘못 올라갔다면 Vercel 설정에서 우회**
1. Vercel 프로젝트 → **Settings → General → Root Directory**
2. **Edit** 클릭 → 실제 프로젝트가 들어있는 하위 폴더 이름 입력 (예: `shipda`)
3. 저장 후 **Redeploy**

둘 중 방법 1이 이후 관리가 더 편합니다.
